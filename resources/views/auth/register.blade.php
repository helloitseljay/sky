<x-guest-layout>
    <x-authentication-card>
        ...
        <form method="POST" action="{{ route('register') }}">
            ...
            <div>
                <x-label class="text-white" for="first_Name" value="First Name" />
            ...
            <div>
                <x-label class="text-white" for="middle_Name" value="Middle Name" />
                <x-input id="middle_name" class="block w-full mt-1" type="text" name="middle_name" :value="old('middle_name')" required autofocus autocomplete="name" />
            </div>
            <div>
                <x-label class="text-white" for="last_name" value="Last Name" />
                <x-input id="last_name" class="block w-full mt-1" type="text" name="name" :value="old('last_name')" required autofocus autocomplete="name" />
            </div>
            ...
            <div class="flex items-center justify-end mt-4">
                <a class="text-sm text-white underline rounded-md hover:text-orange-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500" href="{{ route('login') }}">
                    {{ __('Already registered?') }}
                </a>

                <x-button class="ml-4 text-black bg-orange-600 hover:bg-indigo-600">
                    {{ __('Register') }}
                </x-button>
            </div>
        </form>
